<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100  leading-tight">
                {{ __('Analytics') }}
            </h2>
        </div>
    </x-slot>

    <div class="overflow-x-auto relative shadow-md sm:rounded-lg w-full sm:w-3/4 mx-auto my-6">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <caption class="p-5 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                Analytics
                {{-- @can('create', Spatie\Permission\Models\Appointment::class) --}}
                {{-- <a href="{{ route('analytics.create') }}" class="float-right font-medium text-blue-600 dark:text-blue-500">+</a> --}}
                {{-- @endcan --}}
                <!-- <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">Browse a list of users.</p> -->
            </caption> 
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    <th scope="col" class="py-3 px-6">
                        Statistic Name
                    </th>
                    <th scope="col" class="py-3 px-6">
                        Total items
                    </th>
                    <th scope="col" class="py-3 px-6">
                        Total views
                    </th>

                    <th scope="col" class="py-3 px-6">
                        This week
                    </th>
                    <th scope="col" class="py-3 px-6">
                        details
                    </th>
                    <th scope="col" class="py-3 px-6">
                        <span class="sr-only">Edit</span>
                    </th>
                </tr>

            </thead>
            <tbody>
                @if (count($analytics))
                    @foreach ($analytics as $post)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $post->tracked_name }}
                            </th>

                            <th scope="row"
                                class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $post->total_items }}
                            </th>

                            <td class="py-4 px-6">
                                <span
                                    class="bg-indigo-100 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-900">
                                    {{ $post->total_views }}
                                </span>
                            </td>
                            <th scope="row"
                                class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $post->concurrent }}
                            </th>
                            <th scope="col" class="py-3 px-6">
                                <a class="mx-1 font-medium text-blue-600 dark:text-blue-500 hover:underline"
                                    href="{{route("analytics.details", $post->id)}}">Show history</a>
                            </th>
                            <td class="py-4 px-6 text-right">
                                @can('update', $post)
                                    <a href="{{ route('posts.edit', $post->id) }}"
                                        class="mx-1 font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                @endcan

                                @can('delete', $post)
                                    <form action="{{ route('posts.destroy', $post->id) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="mx-1 font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
                                    </form>
                                @endcan
                            </td>
                        </tr>
                    @endforeach
                @else
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="py-4 px-6 text-center" colspan="4">
                            No analytics found
                        </td>
                    </tr>
                @endif
            </tbody>
        </table>
    </div>



</x-app-layout>
